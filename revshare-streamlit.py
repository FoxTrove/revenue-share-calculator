...
# Define a constant hourly rate for the service provider (not shown to client)
...
def calculate_revenue_share_cap(
    ...
    guarantee_due_months,  # When the guarantee is due (in months)
    ...
    """
    Calculates the Total Payment Cap (the adjusted payment including risk)
    and the Additional Revenue Share (Total Payment minus Contract Value).

    Risk factors are applied only if full contract payment isn’t achieved “timely.”
    ...
    4. **Guarantee Risk:**  
       First, compute the baseline guarantee %:
         baseline_guarantee_pct = (monthly_payment * guarantee_due_months) / total_contract * 100.
       Then, if minimum_guarantee_pct is at or below this baseline, the guarantee risk is fully active:
         guarantee_multiplier = 1 + guarantee_risk_factor * (guarantee_due_months / 12)
       Otherwise, if minimum_guarantee_pct > baseline, then:
         excess_guarantee = minimum_guarantee_pct - baseline_guarantee_pct
         guarantee_effectiveness = excess_guarantee / (100 - baseline_guarantee_pct)
         guarantee_multiplier = 1 + (1 - guarantee_effectiveness) * guarantee_risk_factor * (guarantee_due_months / 12)
       (Thus, a higher minimum guarantee—above the baseline—reduces risk.)
    ...
    *Note:* If full payment is achieved on or before the estimated work duration,
    ...
    delay_months = max(time_to_revenue_share - 1, 0)  # baseline: revenue share kicks in after 1 month
    revenue_delay_multiplier = 1 + delay_months * threshold_multiplier
    ...
    # 4. Guarantee Risk
    baseline_guarantee_pct = (monthly_payment * guarantee_due_months) / total_contract * 100
    # Calculate the guaranteed amount that is "promised" through monthly payments
    # If the minimum guarantee is at or below this baseline, risk is fully active.
    if minimum_guarantee_pct <= baseline_guarantee_pct:
        guarantee_multiplier = 1 + guarantee_risk_factor * (guarantee_due_months / 12)
    else:
        excess_guarantee = minimum_guarantee_pct - baseline_guarantee_pct
        guarantee_effectiveness = excess_guarantee / (100 - baseline_guarantee_pct)
...
**How It Works:**  
1. **Contract Payment Risk:**  
   Calculates the portion of the contract not paid upfront. Less upfront cash increases risk.
2. **Revenue Delay Risk:**  
   Estimates the delay before revenue share kicks in (based on revenue threshold and expected monthly revenue).
3. **Work/Pay Deficit Risk:**  
...
4. **Guarantee Risk:**  
   First, it calculates a baseline guarantee percentage—the percentage of the contract covered by monthly payments over the guarantee period. Guarantees above this baseline reduce risk; guarantees below leave the risk fully active.
5. **Revenue Share Risk:**  
   A lower monthly revenue share percentage increases risk.
6. **Deferred Payment Risk:**  
   Estimates how long the work should take (based on the service provider's monthly earning capacity) and adds risk if the payment period extends beyond that duration.
